...
jpeg(filename = "figure2b.jpeg", width = 1600, height = 500)
...
jpeg(filename = "figure3.jpeg", width = 1500, height = 1500)
(((q2_ah50) / (q2_ah80) / 
...
# plots, AH, observed vs standard
# gs1
dat_plot <- data.frame(time = rep(1:tupper, 2), ah = log(c(ah_observed_gs1, ah_standard_gs1)), 
                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                       Type = rep("Prediction", tupper * 2))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), ah = log(c(ah_observed_gs1, ah_standard_gs1)) - 
                                         1.96 * c(ah_logse_observed_gs1, ah_logse_standard_gs1),
                                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                                       Type = rep("Lower", tupper * 2)))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), ah = log(c(ah_observed_gs1, ah_standard_gs1)) + 
                                         1.96 * c(ah_logse_observed_gs1, ah_logse_standard_gs1),
                                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                                       Type = rep("Upper", tupper * 2)))

jpeg(file = "figure4c.jpeg", height = 400, width = 800)
ggplot(data = dat_plot, aes(x = time, y = exp(ah)*10000, color = Performance, linetype = Type)) + 
  geom_line(size = 1) +
  theme_bw()+
  theme(text = element_text(size=15)) + 
  xlab("Time (days)") + ylab("Harmed cases per 10,000 patients in specialist hospitals") + xlim(c(0, 365)) + 
  scale_linetype_manual(values = c("dotted", "solid", "dashed")) + 
  scale_color_manual(values = my3cols[c(3, 1)])
dev.off()


# plots, AH, observed vs standard
# gs2
dat_plot <- data.frame(time = rep(1:tupper, 2), ah = log(c(ah_observed_gs2, ah_standard_gs2)), 
                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                       Type = rep("Prediction", tupper * 2))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), ah = log(c(ah_observed_gs2, ah_standard_gs2)) - 
                                         1.96 * c(ah_logse_observed_gs2, ah_logse_standard_gs2),
                                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                                       Type = rep("Lower", tupper * 2)))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), ah = log(c(ah_observed_gs2, ah_standard_gs2)) + 
                                         1.96 * c(ah_logse_observed_gs2, ah_logse_standard_gs2),
...
jpeg(file = "figure4a.jpeg", height = 400, width = 800)
ggplot(data = dat_plot, aes(x = time, y = exp(ah) * 10000, color = Performance, linetype = Type)) + 
  geom_line(size = 1) +
  theme_bw() +
  theme(text = element_text(size=15)) + 
  xlab("Time (days)") + ylab("Harmed cases per 10,000 patients in generalist hospitals") + xlim(c(0, 365)) + 
  scale_linetype_manual(values = c("dotted", "solid", "dashed")) + 
  scale_color_manual(values = my3cols[c(2, 1)])
dev.off()

# plots, AAH, observed vs standard
# gs1
dat_plot <- data.frame(time = rep(1:tupper, 2), aah = log(c(aah_observed_gs1, aah_standard_gs1)), 
                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                       Type = rep("Prediction", tupper * 2))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), aah = log(c(aah_observed_gs1, aah_standard_gs1)) - 
                                         1.96 * c(aah_logse_observed_gs1, aah_logse_standard_gs1),
                                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                                       Type = rep("Lower", tupper * 2)))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), aah = log(c(aah_observed_gs1, aah_standard_gs1)) + 
                                         1.96 * c(aah_logse_observed_gs1, aah_logse_standard_gs1),
...
jpeg(file = "figure4d.jpeg", height = 400, width = 800)
ggplot(data = dat_plot, aes(x = time, y = aah, color = Performance, linetype = Type)) + 
  geom_line(size = 1) +
  theme_bw() +
  theme(text = element_text(size=15)) + 
  xlab("Time (days)") + 
  ylab("Relative attributable harm (log) for specialist hospitals") + 
  xlim(c(0, 300)) + 
  scale_linetype_manual(values = c("dotted", "solid", "dashed")) + 
  scale_color_manual(values = my3cols[c(3, 1)])
dev.off()
...
# plots, AAH, observed vs standard
# gs2
dat_plot <- data.frame(time = rep(1:tupper, 2), aah = log(c(aah_observed_gs2, aah_standard_gs2)), 
                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                       Type = rep("Prediction", tupper * 2))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), aah = log(c(aah_observed_gs2, aah_standard_gs2)) - 
                                         1.96 * c(aah_logse_observed_gs2, aah_logse_standard_gs2),
                                       Performance = rep(c("Observed", "Standard"), each = tupper), 
                                       Type = rep("Lower", tupper * 2)))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), aah = log(c(aah_observed_gs2, aah_standard_gs2)) + 
                                         1.96 * c(aah_logse_observed_gs2, aah_logse_standard_gs2),
...
jpeg("figure4b.jpeg", height = 400, width = 800)
ggplot(data = dat_plot, aes(x = time, y = aah, color = Performance, linetype = Type)) + 
  geom_line(size = 1) +
  theme_bw() +
  theme(text = element_text(size=15)) + 
  xlab("Time (days)") + ylab("Relative attributable harm (log) for generalist hospitals") + 
  xlim(c(0, 300)) + 
  scale_linetype_manual(values = c("dotted", "solid", "dashed")) + 
  scale_color_manual(values = my3cols[c(2, 1)])
...
# plots, AH, adjusted
dat_plot <- data.frame(time = rep(1:tupper, 2), ah = log(c(ah_adjusted_gs1, ah_adjusted_gs2)),
                       Hospital = rep(c("Specialist", "Generalist"), each = tupper),
                       Type = rep("Prediction", tupper * 2))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), ah = log(c(ah_adjusted_gs1, ah_adjusted_gs2)) - 
                                         1.96 * c(ah_logse_adjusted_gs1, ah_logse_adjusted_gs2),
                                       Hospital = rep(c("Specialist", "Generalist"), each = tupper),
                                       Type = rep("Lower", tupper * 2)))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), ah = log(c(ah_adjusted_gs1, ah_adjusted_gs2)) +
                                         1.96 * c(ah_logse_adjusted_gs1, ah_logse_adjusted_gs2),
...
jpeg(file = "figure4e.jpeg", height = 400, width = 800)
ggplot(data = dat_plot, aes(x = time, y = exp(ah) * 10000, color = Hospital, linetype = Type)) + 
  geom_line(size = 1) +
  theme_bw() +
  theme(text = element_text(size=15)) + 
  xlab("Time (days)") + 
  ylab("Adjusted harmed cases per 10,000 patients in specialist hospitals") + 
  xlim(c(0, 300)) + 
  scale_linetype_manual(values = c("dotted", "solid", "dashed")) + 
...
# plots, AAH, adjusted
dat_plot <- data.frame(time = rep(1:tupper, 2), aah = log(c(aah_adjusted_gs1, aah_adjusted_gs2)),
                       Hospital = rep(c("Specialist", "Generalist"), each = tupper),
                       Type = rep("Prediction", tupper * 2))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), aah = log(c(aah_adjusted_gs1, aah_adjusted_gs2)) - 
                                         1.96 * c(aah_logse_adjusted_gs1, aah_logse_adjusted_gs2),
                                       Hospital = rep(c("Specialist", "Generalist"), each = tupper),
                                       Type = rep("Lower", tupper * 2)))
dat_plot <- rbind(dat_plot, data.frame(time = rep(1:tupper, 2), aah = log(c(aah_adjusted_gs1, aah_adjusted_gs2)) +
                                         1.96 * c(aah_logse_adjusted_gs1, aah_logse_adjusted_gs2),
...
jpeg(file = "figure4f.jpeg", height = 400, width = 800)
ggplot(data = dat_plot, aes(x = time, y = aah, color = Hospital, linetype = Type)) + 
  geom_line(size = 1) +
  theme_bw() +
  theme(text = element_text(size=15)) + 
  xlab("Time (days)") + ylab("Adjusted relative attributable harm (log)") + xlim(c(0, 300)) + 
  scale_linetype_manual(values = c("dotted", "solid", "dashed")) + 
  scale_color_manual(values = my2cols)
dev.off()
