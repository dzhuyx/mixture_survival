...
llh_fun <- function(Y, d, X0, X1, X2, alpha, beta1, beta2) {
      ...
      temp2 <- sapply(1:n, function(i) {
            if (d[i] == 0) {
                  temp <- 1 - (theta0[i] * Y[i] + theta1[i] / log(theta2[i]) * (theta2[i]^Y[i] - 1))
                  out <- (1 - d[i]) * log(temp)
            } else {
                  out <- 0
            }
...
gof_mle <- function(Y, d, X0, X1, X2, fit0, avec) {
      p <- ncol(X0)
      p1 <- ncol(X1)
      p2 <- ncol(X2)
      n <- length(Y)
            ...
            Mmattemp <- rbind(Mmattemp, do.call("c", lapply(1:n, function(i) {
                  tempfun <- function(t) {
                        out <- (theta00[i] + theta10[i] * theta20[i]^t) /
                              (1 - theta00[i] * t - theta10[i] * (theta20[i]^t - 1) / log(theta20[i]))
                        return(out)
                  }
                  if (Y[i] <= a1) {
                        out <- 0
      ...
      for (j in 1:(length(avec) - 1)) {
            ...
            Cmat <- cbind(Cmat, colMeans(do.call(rbind, lapply(1:n, function(i) {
                  ...
                  fder <- c(X0[i, ] * theta0, theta2^Y[i] * X1[i,] * theta1,
                            theta1* Y[i] * theta2^(Y[i] - 1) * theta2 * (1-theta2) * X2[i, ])
                  Fder <-  c(Y[i] * X0[i, ] * theta0, 
                             (theta2^Y[i] - 1) / log(theta2) * X1[i,] * theta1,
                             theta1 *  (Y[i] * theta2^(Y[i]-1)/log(theta2) - (theta2^Y[i]-1) /theta2/log(theta2)^2) *
                                   theta2 * (1-theta2) * X2[i, ])
